...
class Command:
    ...
    def __init__(self, aliasses: list[str], action, description: str = "—", visible: bool = True) -> None:
        """
...
def print_and_play(message: str) -> None:
    """
...
def show_commands_func(args: list[str]) -> None:
    ...
    """
    print_and_play("Справка")
    for command in commands:
        if command.visible == True:
            rh.play_voice_speech("Команды")
            print_and_play(command.info())
...
def change_func(args: list[str]):
    ...
    if len(args) == 3:
        if args[0] == "модель" and args[1] == "на":
            ...
            if model_name != "":
                if rh.current_model_name != model_name:
                    rh.current_model_name = model_name
                    rh.set_model(
                        rh.Models.big_model
                        if rh.current_model_name == rh.Models.big_model.name
                        else rh.Models.small_model
                    )
                else:
                    print_and_play("Эта модель уже используется")
...
def open_in_browser_func(args: list[str]):
    ...
    querry = "+".join(args)
    cmdCommand = (
        "start microsoftedge https://www.bing.com/search?q=" + querry
    )
...
def incr_decr_prop_func(args: list[str]) -> None:
    ...
    if 3 <= len(args) < 6:
        ...
        if prop == "скорость" or "громкость" and args[1] == "на":
            import NumbersHelper as nh
            import ConfigurationManager as config
            if prop == "скорость":
                nums = [n for n in args[2 : len(args)]]
                value = nh.int_parse(" ".join(nums))
                rh.set_speech_rate(value)
                config.set_value_by_prop_name("speech rate", str(value))


commands = [
    Command(["команды", "помощь", "справка"], show_commands_func, "Справка по командам", False),
    Command(["завершить", "закройся"], exit_func, "Остановить выполнение программы"),
    ...
    Command(["звук"], mute_func, "Блокировать звук. При повторном использовании звук возвращается"),
    Command(["консоль", "терминал"], console_window_func, "Скрыть окно консоли. При повторном использовании окно возвращается"),
    Command(["смени", "измени"], change_func, "Изменить речевую модель"),
    ...
    Command(["увеличь", "прибавь", "уменьши", "убавь"], incr_decr_prop_func, "Увеличить или уменьшить: скорость проигрывания слов / громкость голосового ассистента", False),
]
